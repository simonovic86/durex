// Package storagetest provides a reusable conformance test suite for durex storage backends.
// Import this package from your storage tests to verify interface compliance.
//
// Usage:
//
...
func testFindPending(t *testing.T, store durex.Storage) {
	...
	for i, status := range statuses {
		cmd := &durex.Instance{
			...
			Status:    status,
			CreatedAt: time.Now(),
			ReadyAt:   time.Now().Add(-time.Duration(i) * time.Second),
		}
		if err := store.Create(ctx, cmd); err != nil {
...
func testFindPendingRespectsReadyAt(t *testing.T, store durex.Storage) {
	...
	// Note: Memory and SQLite storage don't filter by ReadyAt in FindPending
